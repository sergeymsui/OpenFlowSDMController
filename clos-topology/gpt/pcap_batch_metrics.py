...
def parse_io_stat(text):
    ...
    for line in text.splitlines():
        if re.search(r"^\s*\d+\.\d+\-\d+\.\d+", line):
            parts = line.split()
            try:
                bits.append(float(parts[-1]))  # последняя колонка = Bits/s
            except Exception:
                pass
    return bits
...
def flows_from_files(files, tcp_port=None):
    ...
    for f in files:
        ...
        if tcp_port:
            cmd = ["tshark","-r",f,"-Y",f"tcp.port=={tcp_port}","-q","-z","conv,tcp"]
        out = run(cmd)
...
def jain(values):
    ...
    s = sum(values); s2 = sum(v*v for v in values); n=len(values)
    if s2==0: return 0.0
    return (s*s)/(n*s2)

def main():
    ap = argparse.ArgumentParser(description="Batch metrics from many PCAPs: MLU, Throughput, FCT, Fairness")
    ap.add_argument("--glob", required=True, help="Glob for PCAPs to aggregate (e.g. '/tmp/*.pcap')")
    ap.add_argument("--capacity-mbps", type=float, required=True, help="Link capacity (Mbps) for MLU on each pcap")
    ap.add_argument("--bin", type=float, default=1.0, help="io,stat bin size in seconds (default 1.0)")
    ap.add_argument("--flows-glob", default=None, help="Glob for PCAPs to compute per-flow FCT/Fairness (e.g. only receiver host pcaps)")
    ap.add_argument("--tcp-port", type=int, default=None, help="Optional TCP port filter for flow stats (e.g., 5201 for iperf3)")
    ap.add_argument("--out-csv", type=Path, default=Path("./pcap_summary.csv"), help="CSV with per-file metrics")
    ap.add_argument("--out-flows", type=Path, default=Path("./flows_summary.csv"), help="CSV with per-flow stats (if flows-glob set)")
    args = ap.parse_args()

    files = sorted(glob(args.glob))
    if not files:
        print(f"[ERROR] no files matched: {args.glob}", file=sys.stderr); sys.exit(1)
    ...
    if args.flows_glob:
        ...
        if not ffiles:
            ...
            per_flow_thr = [(f["bytes"]*8.0)/f["duration"] for f in flows_u if f["duration"]>0]
            fairness = jain(per_flow_thr)
            if durations:
                fct_median = statistics.median(durations)
                fct_p95 = statistics.quantiles(durations, n=100)[94] if len(durations)>=2 else fct_median
            else:
            ...
            with args.out_flows.open("w", encoding="utf-8") as out:
                ...
                for fl in flows_u:
                    ...
                    out.write(f'{fl["src"]},{fl["sp"]},{fl["dst"]},{fl["dp"]},{fl["packets"]},{fl["bytes"]},{fl["duration"]:.6f},{thr:.3f}\n')
