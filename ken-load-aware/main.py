...
class Controller(OSKenApp):
    ...
    def generate_load_aware_flows(self, targets):
        ...
        for dpid, dexed in self.port_stats.items():
            for port_no, tx_rx_bytes in dexed.items():
                #
                for switch_name, switch_params in [
                    ...
                    for source, target, _ in [
                        ...
                        if tx_rx_bytes:
                            (tx_bytes, rx_bytes) = tx_rx_bytes
                            self.topo[source][target]["weight"] = (tx_bytes + rx_bytes)

        for idx, (src, dst) in enumerate(targets):
            simple_path = list( nx.dijkstra_path(self.topo, source=src, target=dst, weight="weight") )

            if simple_path:
                flows[idx] = simple_path
