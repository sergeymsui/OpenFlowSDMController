...
class AutomaticOracle(BaseOracle):
    ...
    def grad(self, t_parameter):
        sorted_vertices = get_tree_order(
            self.graph.nodes_number, self.corr_targets, self.pred_map
        )
        flows = get_flows(
            self.graph.nodes_number,
            self.graph.links_number,
            self.corr_targets,
            self.corr_values,
            self.pred_map,
            sorted_vertices,
    ...
    def update_shortest_paths(self, t_parameter):
        tic = time.time()
        self.distances, self.pred_map = self.graph.shortest_distances(
            self.source, self.corr_targets, t_parameter
        )
...
class PhiBigOracle(BaseOracle):
    def __init__(self, graph, correspondences, processes_number=None):
        ...
        for source, source_correspondences in self.correspondences.items():
            self.auto_oracles.append(
                AutomaticOracle(
                    source, self.graph, source_correspondences, pred_to_edges
                )
            )
        self.time = 0.0
...
@njit
def newton(x_0_arr, a_arr, mu, tol=1e-7, max_iter=1000):
    """
...
class HOracle(BaseOracle):
    def __init__(self, freeflowtimes, capacities, rho=10.0, mu=0.25):
        ...
        with np.testing.suppress_warnings() as sup:
            ...
            self.grad_coef = (
                self.capacities / (self.rho * self.freeflowtimes) ** self.mu
            )
    ...
    def func(self, t_parameter):
        ...
        if self.mu == 0:
            h_func = np.dot(
                self.capacities, np.maximum(t_parameter - self.freeflowtimes, 0)
            )
        else:
            h_func = np.dot(
                self.func_coef,
                np.maximum(t_parameter - self.freeflowtimes, 0) ** (1.0 + self.mu),
            )
    ...
    def conjugate_func(self, flows):
        ...
        if self.mu == 0:
            ...
            return np.dot(
                self.freeflowtimes * flows,
                self.rho
                * self.mu
                / (1.0 + self.mu)
                * (flows / self.capacities) ** (1.0 / self.mu)
                + 1.0,
            )
    ...
    def prox(self, grad, point, A):
        ...
        #      a = bar{f} / (A * bar{t} * rho)
        x = newton(
            x_0_arr=(point_new - self.freeflowtimes) / (self.rho * self.freeflowtimes),
