...
class PrimalDualCalculator:
    def __init__(
        ...
        if mu == 0:
            if base_flows is None:
                ...
                raise ValueError(
                    "Admissible flows should be non-negative and less than capacities"
                )
            else:
    ...
    def duality_gap(self, times, flows):
        if self.mu > 0:
            ...
            admissible_flows = (beta * self.base_flows + self.alpha * flows) / (
                self.alpha + beta
            )
            return self.dual_func_value(times) + self.primal_func_value(
                admissible_flows
            )
    ...
    def get_times(self, flows):
        return self.freeflowtimes * (
            1.0 + self.rho * np.power(flows / self.capacities, 1.0 / self.mu)
        )
