...
from . import transport_graph as tg
from . import oracles
from . import dual_func_calculator as dfc
...
class Model:
    ...
    def __init__(
        self, graph_data, graph_correspondences, total_od_flow, mu=0.25, rho=0.15
    ):
        self.total_od_flow = total_od_flow
        self.mu = mu
        self.rho = rho
        self.inds_to_nodes, self.graph_correspondences, graph_table = self._index_nodes(
            graph_data["graph_table"], graph_correspondences
        )
        self.graph = tg.TransportGraph(
            graph_table, len(self.inds_to_nodes), graph_data["links number"]
        )
    ...
    def _index_nodes(self, graph_table, graph_correspondences):
        ...
        through_nodes = np.unique(
            np.r_[
                table["init_node"][table["init_node_thru"] == True].to_numpy(),
        ...
        for origin, dests in graph_correspondences.items():
            if dests["targets"]:
                correspondences[init_to_ind[origin]] = {
                    "targets": np.array(
                        [term_to_ind[dest] for dest in dests["targets"]],
                        dtype=self.node_type,
                    ),
    ...
    def find_equilibrium(
        self, solver_name="ustm", composite=True, solver_kwargs={}, base_flows=None
    ):
        if solver_name == "fwm":
            ...
            if not "L_init" in solver_kwargs:
                solver_kwargs["L_init"] = (
                    self.graph.max_path_length**0.5 * self.total_od_flow
                )
        ...
        h_oracle = oracles.HOracle(
            self.graph.freeflow_times, self.graph.capacities, rho=self.rho, mu=self.mu
        )
